...
class ConsultSummaryV2ControllerCreateSummaryTest extends ControllerTestSupport {
    ...
    void shouldCreateConsultSummarySuccessfully_whenValidRequestIsProvided() throws Exception {
        ...
        ConsultSummary consultSummary = consultSummaryRepository.findById(resultId).orElseThrow();
    ...
    void shouldReturn404_whenUserIsAlreadyWithdrawnOrDeletedDuringConsultSummaryCreation(UserStatus userStatus)
            throws Exception {
        //given
        UUID userId = UUID.randomUUID();
    ...
    @DisplayName("의료인 성명이 null이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenPractitionerNameIsNullDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());
        ...
        mockMvc.perform(post("/api/v2/consult-summaries")
                ...
                .andExpect(jsonPath("$.isSuccess").value(false))
                .andExpect(jsonPath("$.code").value("COMMON_400"))
                .andExpect(jsonPath("$.result.practitionerName").value("의료인 성명은 필수입니다."));

        verify(transcriptionEventPublisher, never()).publish(any(), any(), any(), any());
    }

    @DisplayName("의료인 성명이 공백이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenPractitionerNameIsBlankDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());
    ...
    @DisplayName("면허 종류가 null이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenLicenseTypeIsNullDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());
        ...
        mockMvc.perform(post("/api/v2/consult-summaries")
                ...
                .andExpect(jsonPath("$.isSuccess").value(false))
                .andExpect(jsonPath("$.code").value("COMMON_400"))
                .andExpect(jsonPath("$.result.licenseType").value("면허 종류는 필수입니다."));

        verify(transcriptionEventPublisher, never()).publish(any(), any(), any(), any());
    }

    @DisplayName("면허 종류가 공백이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenLicenseTypeIsBlankDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());
    ...
    @DisplayName("면허 번호가 null이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenLicenseNumberIsNullDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());

        CreateConsultSummaryV2Request request = createValidRequest().toBuilder().licenseNumber(null).build();
        ...
        mockMvc.perform(post("/api/v2/consult-summaries")
                ...
                .andExpect(jsonPath("$.isSuccess").value(false))
                .andExpect(jsonPath("$.code").value("COMMON_400"))
                .andExpect(jsonPath("$.result.licenseNumber").value("면허 번호는 필수입니다."));

        verify(transcriptionEventPublisher, never()).publish(any(), any(), any(), any());
    }

    @DisplayName("면허 번호가 공백이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenLicenseNumberIsBlankDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());

        CreateConsultSummaryV2Request request = createValidRequest().toBuilder().licenseNumber("  ").build();
    ...
    @DisplayName("소속이 null이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenInstitutionIsNullDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());

        CreateConsultSummaryV2Request request = createValidRequest().toBuilder().institution(null).build();
        ...
        mockMvc.perform(post("/api/v2/consult-summaries")
                ...
                .andExpect(jsonPath("$.isSuccess").value(false))
                .andExpect(jsonPath("$.code").value("COMMON_400"))
                .andExpect(jsonPath("$.result.institution").value("소속은 필수입니다."));

        verify(transcriptionEventPublisher, never()).publish(any(), any(), any(), any());
    }

    @DisplayName("소속이 공백이면 상담 요약 기록 생성에 실패하고, 400을 반환한다.")
    @Test
    void shouldReturn400_whenInstitutionIsBlankDuringConsultSummaryCreation() throws Exception {
        //given
        UUID userId = UUID.randomUUID();
        String accessToken = jwtProvider.generateAccessToken(userId.toString(), Language.KOREAN.name());

        CreateConsultSummaryV2Request request = createValidRequest().toBuilder().institution("  ").build();
